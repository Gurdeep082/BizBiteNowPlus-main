...
export default function Home() {
  ...
  const activeBanners = useMemo(() => {
    ...
    if (festiveDeals && festiveDeals.length > 0) {
      festiveDeals.forEach((d) => {
        const dealProductIds = d.applicable_products || d.product_ids || [];

        let matchedProducts = d.applies_to_all_products
          ? products
          : (dealProductIds.length > 0
            ? products.filter(p => dealProductIds.some(id => String(id) === String(p.id) || String(id) === String(p._id)))
            : []);

        if (matchedProducts.length === 0 && dealProductIds.length > 0 && products.length > 0) {
  ...
  const exclusiveOffersList = useMemo(() => {
    ...
    if (festiveDeals && festiveDeals.length > 0) {
      festiveDeals.forEach(d => {
        const dealProductIds = d.applicable_products || d.product_ids || [];
        let matchedProds = d.applies_to_all_products
          ? products
          : (dealProductIds.length > 0
            ? products.filter(p => dealProductIds.some(id => String(id) === String(p.id) || String(id) === String(p._id)))
            : []);

        if (matchedProds.length === 0 && dealProductIds.length > 0 && products.length > 0) {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[1760px] mx-auto overflow-x-hidden space-y-6 sm:space-y-8 pb-28 font-sans text-slate-800 px-3 sm:px-6 box-border"
    >
      {!isStoreOpen && (
                ...
                {showLoginPopup && (
                  ...
                  >
                    <motion.div
                      ...
                      className="w-full max-w-sm rounded-3xl bg-white p-6 text-center space-y-4 shadow-2xl"
                    >
                      <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-rose-50 text-rose-600">
                        <UserRound size={28} />
                      </div>
                      <div className="space-y-1">
                        <h3 className="text-lg font-semibold text-slate-900">Login Required</h3>
                      ...
                      <div className="flex flex-col gap-2 pt-1">
                        ...
                        <button
                          ...
                          className="w-full rounded-2xl py-3 text-sm font-semibold text-slate-500 hover:bg-slate-100 transition"
                        >
                          Cancel
                        </button>
                      </div>
        ...
        <div className="block sm:hidden w-full">
          <div className="block sm:hidden w-full flex flex-col gap-2.5">
            ...
            {(tables?.length > 0 || !isLoggedIn) && (
              <div className="flex gap-1.5 w-full">
                ...
                <div
                  ...
                  className={`${isLoggedIn ? "w-full" : "w-1/2"} flex items-center justify-between p-2 rounded-2xl bg-white shadow-xs cursor-pointer  min-w-0`}
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-900 shrink-0">
                      <QrCode size={26} />
                    </div>
                  ...
                  <div className="flex items-center gap-2 ml-2 shrink-0">
                    <div className="h-8 w-8 rounded-xl bg-pink-100 flex items-center justify-center text-[var(--primary-color)]">
                ...
                {!isLoggedIn && (
                  <div
                    ...
                    className={`${tables?.length > 0 ? "w-1/2" : "w-full"} bg-white rounded-2xl border border-slate-200 shadow-xs p-2 cursor-pointer flex gap-2 items-center min-w-0`}
                  >
                    <div className="relative shrink-0 flex items-center">
                      <div className="relative">
                        ...
                        <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 bg-white border border-slate-300 rounded px-1 py-[1px] text-[7px] font-bold text-slate-700 shadow-2xs leading-none whitespace-nowrap">
                          Member
                        </div>
                      </div>
                    </div>
            ...
            <div
              ...
              className="w-full flex items-center justify-between p-2.5 sm:p-3 rounded-2xl bg-white shadow-xs cursor-pointer border border-emerald-500/30 hover:border-emerald-400 transition min-w-0"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center shrink-0" style={{ color: "var(--primary-color)" }}>
      ...
      {latestActiveOrder && latestActiveOrder.items && latestActiveOrder.items.length > 0 && (
        <div className="space-y-3 w-full">
          ...
          <div
            ...
            className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-3xl shadow-2xs cursor-pointer transition"
            style={{
        ...
        <div className="space-y-3 w-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-slate-900 text-base sm:text-lg">Top Picks For You</h3>
            </div>
            <button
        ...
        <div className="space-y-3 w-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-slate-900 text-base sm:text-lg">What's Popular & Trending</h3>
            </div>
            <button
              onClick={() => navigate("/customer/menu")}
              className="text-xs font-bold hover:underline cursor-pointer" style={{ color: "var(--primary-color)" }}
        ...
        <div className="space-y-3 w-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-slate-900 text-base sm:text-lg">Gourmet Specials & Quick Bites</h3>
            </div>
            <button
              onClick={() => navigate("/customer/menu")}
              className="text-xs font-bold hover:underline cursor-pointer" style={{ color: "var(--primary-color)" }}
      ...
      {exclusiveOffersList.length > 0 && (
        <div className="space-y-3 w-full">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-slate-900 text-base sm:text-lg">Exclusive Offers</h3>
            <button
              onClick={() => navigate("/customer/rewards")}
        ...
        {selectedOffer && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 sm:p-6 backdrop-blur-sm">
            <motion.div
              ...
              className="relative flex w-full max-w-lg flex-col overflow-hidden rounded-[28px] sm:rounded-3xl bg-white shadow-2xl max-h-[85vh]"
            >
              <button
              ...
              {selectedOffer.image && (
                <div className="relative h-40 sm:h-48 w-full shrink-0 bg-slate-100">
                  <img
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 sm:space-y-6 scrollbar-hide">
                ...
                {selectedOffer.products && selectedOffer.products.length > 0 && (
                  <div className="space-y-2.5 pt-1">
                    <h4 className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-slate-400">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {selectedOffer.products.map((product) => (
                        <div
                          key={product.id}
                          className="flex items-center gap-3 rounded-2xl border border-slate-200/80 bg-white p-2 sm:p-2.5 shadow-2xs"
                        >
                          <img
                            src={product.image}
                            alt={product.name}
                            className="h-10 w-10 sm:h-12 sm:w-12 shrink-0 rounded-xl object-cover bg-slate-100"
                          />
                          <div className="min-w-0 flex-1">
                            <h5 className="truncate text-[11px] sm:text-xs font-bold text-slate-900">
                              {product.name}
                            </h5>
                            <p className="mt-0.5 text-[11px] font-black" style={{ color: "var(--primary-color)" }}>
                              ₹{product.price}
                            </p>
                          </div>
                        </div>
              ...
              <div className="shrink-0 border-t border-slate-100 bg-white p-3.5 sm:p-5 shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)]">
                <button
                  onClick={async () => {
                    if (!isStoreOpen) {
