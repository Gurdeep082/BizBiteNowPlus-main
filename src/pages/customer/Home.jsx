...
const MOCK_STORE_INFO = { is_open: true, business_info: { business_type: "Restaurant" } };
...
export default function Home() {
  ...
  const storefront = mockProducts;
  const storeInfo = MOCK_STORE_INFO;
  const festiveDeals = mockFestiveDeals;
  const publicBanners = mockBanners;
  ...
  const orders = useOrderStore((s) => s.orders.length ? s.orders : mockOrders);
  const tables = mockTables;
  const stampsCollected = 4;
  const threshold = mockLoyaltySettings.target_stamps;

  const [liveDiscounts] = useState(mockDiscounts);
  ...
  const [rewardCoupons] = useState(mockRewardCoupons);
  const settingsData = mockLoyaltySettings;
        ...
        {selectedOffer.image && (
          <div className="relative h-40 sm:h-48 w-full shrink-0 bg-slate-100 dark:bg-white/10">
            ...
            <span
              ...
              style={{ color: "var(--primary-color)" }}
            >
              {selectedOffer.tag || selectedOffer.title || "Offer"}
            </span>
          </div>
        )}
